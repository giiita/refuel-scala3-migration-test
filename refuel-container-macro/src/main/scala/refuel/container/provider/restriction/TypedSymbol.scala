...
private[refuel] case class TypedSymbol[T](
                                                 ...
                                                 acceptedFrom: Vector[Class[_]]) extends SymbolRestriction[T](key, value, priority) {
  /**
   ...
   * @return
   */
  override def acceptedClass[X](x: Class[X]): Boolean = acceptedFrom.exists(_.isAssignableFrom(x))
